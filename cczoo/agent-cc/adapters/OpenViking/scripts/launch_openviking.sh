...
prepare_openviking_storage() {
    export OPENVIKING_DOCKER_CMD="docker run -d --name=${OPENVIKING_CONTAINER} --label=argus.workload=${WORKLOAD_ID} --env=OPENVIKING_CONFIG_FILE=${OPENVIKING_CONTAINER_STATE_DIR}/ov.conf --env=OPENVIKING_WITH_BOT=0"
