...
## Host-managed OpenViking Workload Attestation

For metadata `workload_attestation_profile=nginx-spiffe-helper-v1`, set
`ARGUS_WORKLOAD_CONFIG` to the absolute path of the root-owned, protected
`tc-api-workload.json` generated by the [Workload deployment tool](../spire/workload/README.md).
The file contains only `schema_version: 1` and the `workload` section of the
operator configuration. Mount it read-only into TC API; also expose the configured
host configuration/data paths at the same paths visible to the Docker daemon.
There is no fallback to earlier per-path environment variables.

The requested workload ID must match the configured ID. The profile keeps
OpenViking-specific configuration checks, loopback-only business listening,
read-only rootfs/configuration, a separate writable data mount, and bridge
network isolation. The configured ports and paths drive both Docker arguments
and the transparency log's mounts, published ports and environment digest from
