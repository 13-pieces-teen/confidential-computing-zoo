...
uses `argus-spire-evidence-provider`, external Agent and Server NodeAttestor plugins,
...
A caller cannot select the Provider's identity through `/ra/v1/node-evidence`.
...
The same Provider keeps the separate `POST /ra/v1/workload-evidence` binding
contract when configured with a protected workload registration file. That
path continues to require
`spiffe://argus.local/spire/agent/argus_tdx/openviking-node`; it does not inherit
arbitrary Node identity configuration. The Provider rejects a different
Agent ID when workload registration is enabled.
...
the separate, implemented OpenViking workload-attestation stage. The combined
deployment retains `spiffe://argus.local/spire/agent/argus_tdx/openviking-node`
as the parent identity for its existing Workload binding, entries, and policies.
...
Argus v2 reuses the Intel Guard binary and selects
`GUARD_MODE=spiffe_identity` explicitly. In this mode SPIRE and mTLS establish
the live caller and target workload identities, while Guard evaluates the
caller-local YAML policy through `POST /guard/v1/authorize`. It does not call
the evidence endpoints and does not weaken or wrap the Intel evidence flow.

The two modes expose separate decision routes:

| Guard mode | Decision route | Trust input |
|------------|----------------|-------------|
| `evidence` (default) | `/ra/v1/verify`, `/ra/v1/verify/batch` | TDX evidence, request binding, Intel CA, strict policy |
| `spiffe_identity` | `/guard/v1/authorize` | mTLS-authenticated SPIFFE identities and caller-local YAML policy |

Both modes expose `/health`. Only `spiffe_identity` exposes `/metrics`. A Guard
listening outside loopback requires a bearer token supplied through
`ARGUS_API_TOKEN` or `ARGUS_API_TOKEN_FILE`.
