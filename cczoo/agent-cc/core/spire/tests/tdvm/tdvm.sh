...
ACTION="${1:-status}"
TDVM_PROFILE="${TDVM_PROFILE:-openviking}"
[[ "$TDVM_PROFILE" == openviking || "$TDVM_PROFILE" == openclaw ]] || { echo 'Unknown TDVM_PROFILE' >&2; exit 1; }
TDVM_NAME="${TDVM_NAME:-argus-$TDVM_PROFILE-tdx}"
...
if [[ "$TDVM_PROFILE" == openclaw ]]; then
    TDVM_SSH_PORT="${TDVM_SSH_PORT:-2223}"
else
...
prepare() {
    ...
    require_images
    if running_pid >/dev/null; then fail 'refusing to inspect or modify a running VM disk'; fi
    [[ "$(realpath -m "$TDVM_BASE_IMAGE")" != "$(realpath -m "$TDVM_OVERLAY_IMAGE")" ]] || fail 'base and overlay must differ'
...
start() {
    local command_line docker_mtls_bind_address netdev
    local -a ports=("$TDVM_SSH_PORT")

    require_command qemu-system-x86_64
    require_command ss
    netdev="user,id=net0,hostfwd=tcp:127.0.0.1:$TDVM_SSH_PORT-:22"
    docker_mtls_bind_address=""
    if [[ "$TDVM_PROFILE" == openviking ]]; then
    ...
    netdev+=",hostfwd=tcp:127.0.0.1:$TDVM_OPENVIKING_PORT-:1933,hostfwd=tcp:127.0.0.1:$TDVM_MTLS_PORT-:1943,hostfwd=tcp:$docker_mtls_bind_address:$TDVM_MTLS_PORT-:1943"
    ports+=("$TDVM_OPENVIKING_PORT" "$TDVM_MTLS_PORT")
    fi
    mkdir -p "$TDVM_RUNTIME_DIR"
    if pid="$(running_pid)"; then
        command_line="$(ps -p "$pid" -o args=)"
        [[ "$command_line" == *"$netdev"* ]] || fail 'running TDVM network differs from requested profile'
        printf '%s\n' "$pid" >"$PID_FILE"
    ...
    for _ in $(seq 1 60); do
        ...
        if ss -ltn "sport = :$TDVM_SSH_PORT" | grep -q LISTEN; then
            printf 'TD VM started: profile=%s pid=%s ssh_forward=127.0.0.1:%s\n' "$TDVM_PROFILE" "$pid" "$TDVM_SSH_PORT"
            printf 'SSH forwarding is listening; Guest boot and attestation have not been verified.\n'
            return
        fi
        read -r -t 1 _ || true
    done
