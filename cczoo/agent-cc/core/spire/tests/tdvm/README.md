...
M4 predates the formal asymmetric runtime under `../../runtime/asymmetric/`. It is retained as a
...
```bash
TDVM_SSH_IDENTITY=/path/to/tdvm-key core/spire/tests/tdvm/test-architecture.sh
```
...
core/spire/tests/tdvm/deploy-openviking-tdvm.sh deploy
core/spire/tests/tdvm/deploy-openviking-tdvm.sh status
```
...
```bash
core/spire/tests/tdvm/deploy-openviking-tdvm.sh rollback
```
