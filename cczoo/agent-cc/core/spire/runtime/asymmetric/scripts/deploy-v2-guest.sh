...
start_workload() {
    ...
    remote_sudo /usr/local/bin/docker inspect "$OPENVIKING_CONTAINER" >/dev/null \
        || fail "native OpenViking container is missing: run adapters/OpenViking/scripts/launch_openviking.sh in the Guest with OPENVIKING_SPIFFE_ENABLED=1 and OPENVIKING_SPIFFE_WORKLOAD_API_DIR=$REMOTE_RUN"

    local spiffe_enabled socket_mount running
    running="$(remote_sudo /usr/local/bin/docker inspect "$OPENVIKING_CONTAINER" --format '{{.State.Running}}')"
    [[ "$running" == true ]] || fail 'native OpenViking container is not running'
    spiffe_enabled="$(remote_sudo /usr/local/bin/docker inspect "$OPENVIKING_CONTAINER" --format '{{range .Config.Env}}{{println .}}{{end}}')"
    [[ "$spiffe_enabled" == *$'ARGUS_SPIFFE_ENABLED=1\n'* || "$spiffe_enabled" == ARGUS_SPIFFE_ENABLED=1* ]] \
        || fail 'native OpenViking container was not launched with ARGUS_SPIFFE_ENABLED=1'
    socket_mount="$(remote_sudo /usr/local/bin/docker inspect "$OPENVIKING_CONTAINER" --format '{{range .Mounts}}{{if eq .Destination "/opt/spire/run/openviking"}}{{.Source}}{{end}}{{end}}')"
    [[ "$socket_mount" == "$REMOTE_RUN" ]] \
        || fail "native OpenViking Workload API mount is $socket_mount, expected $REMOTE_RUN"

    for _ in $(seq 1 30); do
        if remote_sudo /usr/local/bin/docker exec "$OPENVIKING_CONTAINER" \
            grep -q '"spiffe_id": "spiffe://argus.local/service/openviking-cmem"' \
            /run/argus-svid/status.json >/dev/null 2>&1; then
            printf 'OpenViking native SPIFFE mTLS workload is ready on TDVM port 1943.\n'
            return
        fi
        read -r -t 1 _ || true
    done
    remote_sudo /usr/local/bin/docker logs --tail 80 "$OPENVIKING_CONTAINER" >&2 || true
    fail 'OpenViking workload did not receive its X.509-SVID'
}

status() {
    remote test -c /dev/tdx_guest || fail 'SSH target is not a TD Guest'
    for container in "$PROVIDER_CONTAINER" "$AGENT_CONTAINER" "$OPENVIKING_CONTAINER"; do
